...
## Session: Bug Hunt 2026-06-28
1. **Match arm guards ignored** (`codegen.c`): Guards in `Some(y) if y > 0` were parsed and stored in `arm->match_arm.guard` but never evaluated by codegen. Fixed by adding guard eval after tag match in both payload and no-payload paths, jumping to next_arm when guard is false.
2. **`consume()` memory leak** (`parser.c`): On expected-token mismatch, `next()` allocated a token string that was never freed. Fixed by calling `lexer_free_token(&t)` before `memset`.
3. **`&(expr.field)` borrow returns wrong address** (`codegen.c`): `&p.x` loaded the field *value* instead of computing the field's *address*. Fixed by detecting `NODE_BORROW` on `NODE_INDEX`, emitting field offset LEA.
4. **Comprehension `stack_size` double-count** (`codegen.c`): `count_locals` for `NODE_COMPREHENSION` called `sym_add` (incrementing `stack_size`) and also manually did `s->stack_size += 8`, double-allocating. Fixed by removing the manual `+= 8`.
5. **Assign `is_heap` loses track** (`codegen.c`): `NODE_ASSIGN` always reset `is_heap[idx]=0` for non-struct/enum RHS, even when RHS was an ident pointing to a heap variable. Fixed by preserving `is_heap` when RHS is `NODE_IDENT`.
6. **`ast_fold` infinite loops in for-each** (`mir_opt.c`): `NODE_BLOCK` for-loop reassigned `s` to fold result, corrupting the linked-list traversal (IF folding replaces node). Fixed by saving `next` before folding. Same issue in `NODE_CALL` args loop — changed to discard return value.
7. **`loop_sp` not reset between functions** (`codegen.c`): `gen_fn` reuses the codegen but never reset `c->loop_sp`, causing stack overflow from nested-loop tracking accumulating across functions. Fixed by adding `c->loop_sp = 0` at top of `gen_fn`.
8. **Shifts in LIR lowered as MOV** (`lir/lir_lower.c`): Opcodes 16 (SHL) and 17 (SHR) in MIR binary ops emitted `LIR_MOV` instead of `LIR_SHL`/`LIR_SHR`. Fixed by emitting the correct shift instruction after MOV.
9. **Lexer string token length off-by-one** (`lexer.c`): String `t.len` computed as `pos - start + 1`, overcounting by 1. Fixed to `t.len = l->pos - start`.
10. **Match arm payload stack under-allocation** (`codegen.c`): `count_locals` did not call `sym_add` for match arm payload idents, so stack frame was under-allocated. Codegen's `gen_stmt` called `sym_add` at runtime but the slot was beyond the frame (red zone), clobbered by function calls in the body. Fixed by pre-allocating payload slots via `sym_add` in `count_locals` and using `sym_find_idx` + `c->sym.offsets[pidx]` in codegen instead of creating a new entry.
11. **DCE `count_uses` holes** (`mir_opt.c`): `count_uses` did not handle `NODE_DEREF`, `NODE_BORROW`, `NODE_MUT_BORROW`, `NODE_COMPREHENSION`, `NODE_STRUCT_LITERAL`, `NODE_ENUM_LITERAL`. Variables used in these constructs would appear unused to DCE, potentially causing incorrect dead let elimination. Fixed by adding recursive traversal for all missing node types.
12. **DCE `ast_dce_block` doesn't recurse into match arms** (`mir_opt.c`): Dead `let` bindings inside match arm bodies would never be eliminated. Fixed by adding `NODE_MATCH` case that recurses into each arm's body.
13. **`count_locals` enum_literal payload iteration** (`codegen.c`): `count_locals` for `NODE_ENUM_LITERAL` only called itself on the first payload node, missing subsequent payload entries. Fixed by iterating the linked list via `for (Node *p = ...; p; p = p->next)`.
